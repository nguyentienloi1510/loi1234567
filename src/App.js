...
function App() {
  ...
  const onHandleAddProduct = async (product) => {
    try {
        const {data:pro}= await ProductAPI.add(product); // côn api
      setProducts([
        ...products,
        pro
      ]); // rereder
...
// bất đồng bộ
  ...
  const onHandleDeleteProduct = async (id) => {
    try {
      await ProductAPI.remove(id);// xóa trên api
      const newProducts = products.filter(product => product._id !== id); // lấy id và xóa 
      setProducts(newProducts); // rereder
    } catch (error) {
      console.log(error)
    }
  }
  ...
  return (
    <div className="">
      ...
      onAdd={onHandleAddProduct} // props
